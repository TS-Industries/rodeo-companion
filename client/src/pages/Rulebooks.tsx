...
const NHSRA_SECTIONS = [
  ...
  },
  {
    ...
    content: `Breakaway roping is a timed event for female contestants. The rope must break away cleanly from the saddle horn. A white string or flag must be tied to the end of the rope and to the saddle horn. Time is taken when the rope breaks free. Ten-second time limit applies. Barrier penalty of 10 seconds for breaking early. No time for catching by the neck only in some associations. Legal catches: around the neck. The rope must completely encircle the calf's neck. No crossfire catches. Proper western attire required.`
  },
  {
    title: "Goat Tying Rules",
    content: `Goat tying is a timed event for female contestants and junior high contestants. Contestant starts mounted, rides to goat, dismounts, catches, throws and ties goat. Three legs must be crossed and tied. Tie must hold for six seconds after contestant removes hands and backs away. No assistance from horse in throwing or holding the goat. Time starts when contestant crosses the score line. Contestant must have horse under control and tied or hobbled. Proper form: run to goat, throw by hand, cross and tie three legs.`
  },
  {
    title: "Tie Down Roping Rules",
    content: `Tie down roping is a timed event. Contestant must rope calf, dismount, go to calf, and tie three legs. Tie must hold six seconds after signal is given. Calf must be thrown by hand — if calf is down when roper reaches it, calf must be let up and thrown by hand. Barrier: 10-second penalty for breaking early. Thirty-second time limit. Legal catch is around any part of the calf. Three-wrap tie with a half-hitch. Neck rope required on horse. Rope must remain on calf until tie is passed on. No jerking of calf after tie.`
  },
  {
  ...
  },
  {
    ...
    content: `Ribbon roping is a team event with a boy roper and a girl runner. The boy ropes the calf; the girl removes a ribbon from the calf's tail and runs across the finish line. Time starts when calf leaves chute. Time ends when girl crosses finish line with ribbon. Barrier penalty applies to the roper. Legal catches include any legal roping catch. The ribbon must be clearly removed and the girl must cross the finish line. Both team members must be of high school or junior high age.`
  },
  {
    title: "Chute Dogging Rules",
    content: `Chute dogging is a junior high event similar to steer wrestling but without a horse. Contestant starts in the chute with the steer and must throw the steer from the chute. Steer must be brought to a complete stop and thrown. Same down rules as steer wrestling apply. Time limit: 30 seconds. Protective equipment recommended. Event is designed for younger or smaller contestants to learn wrestling fundamentals.`
  },
  {
...
const WRA_SECTIONS = [
  ...
  },
  {
    ...
    content: `For the complete and most current WRA rulebook, please visit the official Wildrose Rodeo Association website. The rulebook is updated regularly and the official version supersedes any summary provided here. Trades can be done up until Noon on Mondays following callback — email the office for trade requests.`
  },
];
...
export default function Rulebooks() {
  ...
  const assocInfo: Record<Association, { name: string; type: string; color: string }> = {
    LRA: { name: "Lakeland Rodeo Association", type: "Amateur", color: "oklch(0.65 0.18 25)" },
    NHSRA: { name: "Natl. High School Rodeo Assoc.", type: "High School / Junior", color: "oklch(0.60 0.15 250)" },
    WRA: { name: "Wildrose Rodeo Association", type: "Amateur", color: "oklch(0.55 0.15 145)" },
  };
      ...
      <div className="px-4 pt-2">
        {matchCount === 0 && query.trim() ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm mt-1" style={{ color: "oklch(0.50 0.04 60)" }}>
